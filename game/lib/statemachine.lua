...
-- Register bitser extension for statemachine serialization
if bitser then
  bitser.registerExtension("statemachine", {
    ['bitser-type'] = "table",
    ['bitser-match'] = function(obj)
      -- Check if this is a statemachine object
      return type(obj) == "table" and
          obj.current and
          obj.events and
          getmetatable(obj) == machine
    end,
    ['bitser-dump'] = function(obj)
      -- Only serialize the essential state data, not callbacks
      return {
        current = obj.current,
        asyncState = obj.asyncState,
        options = obj.options,
        events = obj.events
      }
    end,
    ['bitser-load'] = function(data)
      -- Recreate the statemachine from the options
      local fsm = machine.create(data.options)
      -- Restore the current state
      fsm.current = data.current
      fsm.asyncState = data.asyncState
      return fsm
    end
  })
end
